...
function showCategoriesList(name, array) {
  /* funcion para mostrar los items con imagen, precio, nombre, currency y cantidad vendidos*/
  ...
  .then((data) => {
    ...
    document
      ...
      .addEventListener("click", function () {
        if (
          rangeFilterCountMin.value.trim() !== "" ||
          rangeFilterCountMax.value.trim() !== ""
        ) {
    ...
    document.getElementById("query").addEventListener("input", function (e) {
      ...
      if (value.length > 0) {
        const searched = productsArray.filter(
          (product) =>
